...
def choose_color():
    ...
    userChosenLabel = Label(root,text="User Chosen Color").pack()
    userColorFrame = Frame(root, bg=color_code[1],width=1000, height=100).pack()
    userChosenLabel2 = Label(root,text=str(color_code[1])).pack()

    #If suggesting 1 color
    if(schema.get() == "complimentary"):
        #suggestedColorLabel1.pack_forget()
        suggestedColorLabel1 = Label(root,text="Suggested Color #1:").pack()
        suggestedColorFrame1 = Frame(root, bg=hls2hex(b[0]),width=1000, height=100).pack()
        suggestedColorLabel2 = Label(root,text=str(hls2hex(b[0]))).pack()
    ...
    if(schema.get() == "tetratic"):
        ...
        suggestedColorLabel5 = Label(root,text="Suggested Color #3:").pack()
        suggestedColorFrame3 = Frame(root, bg=hls2hex(b[2]),width=1000, height=100).pack()
        suggestedColorLabel6 = Label(root,text=str(hls2hex(b[2]))).pack()
...
# Create label
label = Label(root, text="Pick a color by clicking on the button below:").pack()
#label.grid(row=0)
...
#button = Button(root , text = "Update Suggestions" , command = updateThemeOrSchema).pack()
